...
RUN bash -c 'ARCHIVE="grafana-${GRAFANA_VERSION}.linux-${TARGETARCH}.tar.gz" && \
    ...
    cd grafana && \
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-exploretraces-app 0.2.8 && \
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-lokiexplore-app 1.0.10 && \
