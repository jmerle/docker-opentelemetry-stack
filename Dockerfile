...
ENV VICTORIA_METRICS_VERSION=1.128.0
...
ENV OPENTELEMETRY_COLLECTOR_VERSION=0.138.0
...
RUN bash -c 'ARCHIVE="grafana-${GRAFANA_VERSION}.linux-${TARGETARCH}.tar.gz" && \
    ...
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-lokiexplore-app 1.0.30 && \
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-clock-panel 3.0.1 && \
