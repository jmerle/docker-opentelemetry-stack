...
ENV VICTORIA_METRICS_VERSION=1.130.0
...
ENV OPENTELEMETRY_COLLECTOR_VERSION=0.139.0
...
RUN bash -c 'ARCHIVE="grafana-${GRAFANA_VERSION}.linux-${TARGETARCH}.tar.gz" && \
    ...
    cd grafana && \
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-exploretraces-app 1.2.0 && \
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-lokiexplore-app 1.0.31 && \
    ./bin/grafana cli --pluginsDir /data/grafana/plugins plugins install grafana-clock-panel 3.1.0 && \
